...
# -------------------------
# A compact PAKISTANI-flavored catalog with attributes: id, name, price (PKR), category, color, sizes
CATALOG = [
    {
        "id": "mug-001",
...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at SAUD'S Shop:"]
...
def create_order_object(line_items: List[Dict], currency: str = "PKR") -> Dict:
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at  SAUD'S Shop:"]
...
async def show_cart(
    ...
    for li in userdata.cart:
        ...
        lines.append(f"- {p['name']} x {li['quantity']}{sz_text}: {line_total} PKR")
    lines.append(f"Cart total: {total} PKR")
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'SAUD SONIJA', the friendly shopkeeper and voice assistant for SAUD'S Shop.
        Universe: A small neighbourhood PAKISTANI shop selling mugs, hoodies and tees.
        ...
        Rules:
            ...
            - When presenting options, include product id and price (e.g. 'mug-001 — 299 PKR').
        """
        super().__init__(
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT  — SAUD SONIJA")
