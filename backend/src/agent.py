# ======================================================
# 💼 DAY 6: AI FRAUD DETECTION AGENT (UPDATED)
# 🛡️ "Naya Pay Security Desk" - Card Flag Agent
# 🚀 Primary Goal: Check User's Card Ending against Fraud Case Database
# ======================================================
...
def load_fraud_database():
    ...
    try:
        if not os.path.exists(db_path):
            print(f"⚠️ FRAUD DB not found at {db_path}. Using empty list.")
            return {}
        ...
        print(f"⚠️ Error loading Fraud DB: {e}")
        return {}
...
async def check_for_fraud_flag(
    ...
    """
    print(f"🔎 CHECKING CARD ENDING: {card_ending}")
    ...
    if fraud_record:
        ...
        if status == "confirmed_fraud":
            print(f"🚨 CONFIRMED FRAUD DETECTED for {card_ending}.")
            # Return a clear, actionable instruction for the LLM
            return (f"🚨 CONFIRMED FRAUD: User {userName} is associated with a confirmed fraud case. "
             ...
             print(f"⚠️ PENDING REVIEW DETECTED for {card_ending}.")
             return (f"⚠️ PENDING REVIEW: A suspicious transaction is pending review for user {userName}. "
    ...
    print(f"✅ ACCOUNT SAFE: {card_ending}")
    return "✅ ACCOUNT SAFE: This card ending has no outstanding fraud flags. Tell the user they can proceed with their request."
...
async def entrypoint(ctx: JobContext):
    ...
    print("🚀 STARTING NAYA PAY SECURITY SESSION")
